<?php 
 ...
 if(filter_has_var(INPUT_POST, "ROOT_PATH")){
     ...
     //缓存设置
     $CONFIG["cache"]=array ( 'type' =>filter_input(INPUT_POST, "cache_type"),'ip' =>filter_input(INPUT_POST, "cache_ip"),'pass' =>filter_input(INPUT_POST, "cache_pass"),'prot' =>filter_input(INPUT_POST, "cache_prot"),'prefix'=>filter_input(INPUT_POST, "cache_prefix"), 'time' =>filter_input(INPUT_POST, "cache_time"),); 
...
if(filter_has_var(INPUT_POST, "resource")){
     $input=trim(filter_input(INPUT_POST, "resource"));
     if($input===""){
         $CONFIG["resource"]=[];        
     } else{
        $arr=preg_split('/[\r\n]+/s',$input );	
        foreach($arr as $key){$val=explode(",",$key); $array[]=array('name'=>$val[0],"url"=>$val[1]);}
...
if(filter_has_var(INPUT_POST, "parse")){
     $input=trim(filter_input(INPUT_POST, "parse"));
     if($input===""){
         $CONFIG["parse"]=[];        
     } else{
        $arr=preg_split('/[\r\n]+/s',$input );	
...
//直播设置
if(filter_has_var(INPUT_POST, "live")){
     $input=trim(filter_input(INPUT_POST, "live"));
     if($input===""){
         $CONFIG["live"]=[];        
     } else{
        $arr=preg_split('/[\r\n]+/s',$input );	
        foreach($arr as $key){$val=explode(",",$key); $array[]=array('name'=>$val[0],"url"=>$val[1],"play"=>$val[2]?$val[2]:"default");}
        $CONFIG["live"]=$array;
    }
...
//影片设置
if(filter_has_var(INPUT_POST, "vod")){
     $input=trim(filter_input(INPUT_POST, "vod"));
     if($input===""){
         $CONFIG["vod"]=[];        
     } else{
        $arr=preg_split('/[\r\n]+/s',$input );	
        foreach($arr as $key){$val=explode(",",$key); $array[]=array('name'=>$val[0],"url"=>$val[1],"data"=>$val[2]);}
        $CONFIG["vod"]=$array;
    }
...
if(filter_has_var(INPUT_POST, "play_off_link")){	
	...
	$CONFIG["play"]['off']['link']=filter_input(INPUT_POST, "play_off_link");        
        ...
        $CONFIG["play"]['off']['allyun']=filter_input(INPUT_POST, "play_off_allyun");
        $CONFIG["play"]['off']['allsearch']=filter_input(INPUT_POST, "play_off_allsearch");
}
//播放线路配置
...
if(filter_has_var(INPUT_POST, "play_style_logo_show")){	
	...
	$CONFIG["play"]['style']['logo_show']=filter_input(INPUT_POST, "play_style_logo_show");
        ...
        $CONFIG["play"]['style']['off']=filter_input(INPUT_POST, "play_style_off");
         file_put_contents("../save/play.css",base64_decode(filter_input(INPUT_POST, "play_style_css")));
