/*!
 * Xyplayer.js v4 (2021.07.02)
 * (c) 2014-2021 xymov.net
 * Released under the Commercial License. */
 /*  global self,define,opera, java, Base64, _GET, top */
    ...
    xyplayer.prototype = {
       ...
       setTitle:function (title) {
        ...
        liveplay: function (num, nosave) {
            nosave = nosave || false;
            if ("undefined" !== typeof this.play.define.live && num !== "") {
                var url = this.play.define.live[num].url;
                try {
                    top.document.title = "正在直播:【" + this.play.define.live[num].name + "】" + "--" + this.mytitle;
                } catch (error) {
                    document.title = "正在直播:【" + this.play.define.live[num].name + "】" + "--" + this.mytitle;
                }

                this.live_num = num;
                this.relive();
                if (!nosave) {
                    setCookie("live_num", num, $this.timecookie);
                } else {
                    setCookie("live_num", "", $this.timecookie);
                }
                var play=this.play.define.live[num].play==='default'||this.play.define.live[num].play==='' ? this.myplayer:this.play.define.live[num].play;
     ...
     //刷新直播
        relive: function () {
            $(this.plive).empty();
            for (var key in $this.play.define.live) {
                if($this.play.define.live[key].off==='0'){continue;};

                if (key === this.live_num && 'live' === this.onflag) {
                    $(this.plive).append('<li class="click_work " style="' + this.play.style.line_on + '" type="live" num="' + key + '">' + $this.play.define.live[key].name + '√</li>');
                } else {
                    $(this.plive).append('<li class="click_work" type="live" num="' + key + '">' +$this.play.define.live[key].name + '</li>');
                }
            };
        },
        ...
        echo: function (word, lslogo, lsurl, obj) {
            ...
            if (lsurl) {
                ...
                this.farme(obj, "play.html?url=" + encodeURIComponent(word),this.success);
